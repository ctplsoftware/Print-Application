...
<form action="/reprint-update/{{$reprintData->label_name}}" method="POST" enctype='multipart/form-data'
        ...
        <div class="card" style="padding:17px; overflow-y: scroll; overflow-x: hidden;max-height: 560px;">
            ...
            <div class="headingfont form-row ">
                <div class="form-group col-md-3" style="">
                    ...
                    <input name="label_name" readonly required id="label_name"
                        class=" form-control validate required form-control-sm" value="{{$designlabel->label_name}}">
                </div>
                <div class="form-group col-md-3" style="">
                    <label>Product Type</label>
                    <input type="text" class=" form-control validate required form-control-sm" id="product_type" name="product_type"
                        value="{{$labelProductName->product_type}}" readonly>
                </div>
                <div class="form-group col-md-3" style="">
                    <label> Label Type</label>
                    <input type="text" class=" form-control validate required form-control-sm" name="label_type"
            ...
            <div class="headingfont form-row ">
                <div class="form-group col-md-3 {{$config['product_name_use'] == 'off' ? '' : 'hideField'}}">
                    ...
                    <!-- <select name="product_name" id="product_name" class="form-control validate required form-control-sm"
                        {{$config['product_name_mandatory'] == 'on' ? 'required' : ''}} readonly>
                        <option value="{{$product_data->product_name}}" selected readonly>{{$reprintData->product_name}}
                        </option>
                ...
                <div class="form-group col-md-3 {{$config['date_of_manufacturing_use'] == 'on' ? '' : 'hideField'}}">
                    ...
                    <div class="input-row">
                        <input type="text" maxlength="100" name="date_of_manufacturing" id="date_of_manufacturing"
                            ...
                            class="restrict required validate input-large form-control form-control-sm enablefields"
                            autocomplete="off" value="{{$reprintData->date_of_manufacturing}}"
                            {{$config['date_of_manufacturing_mandatory'] == 'on' ? 'required' : ''}} readonly/>
                        <input id="future_date_validate" type="{{$config['date_format'] == 'MMM-YYYY' ? 'month' : 'date'}}"
                            class=" form-control form-control-sm input-small enablefields" onChange="DateChange(this)" readonly/>
                    </div>
            </div>
            <div class="form-group col-md-3 {{$config['date_of_expiry_use'] == 'on' ? '' : 'hideField'}}">
                    ...
                    <div class="input-row">
                        <input type="text" maxlength="100" name="date_of_expiry" id="date_of_expiry"
                            class="restrict required validate input-large form-control form-control-sm enablefields"
                            autocomplete="off" placeholder="{{$config['date_format']}}" value="{{$reprintData->date_of_expiry}}"
                            {{$config['date_of_expiry_mandatory'] == 'on' ? 'required' : ''}} readonly/>
                        <input type="{{$config['date_format'] == 'MMM-YYYY' ? 'month' : 'date'}}"
                            class=" form-control form-control-sm input-small past_date_validate enablefields" onChange="DateChange(this)" readonly/>
                    </div>
            </div>
                <div class="form-group col-md-3 {{$config['date_of_retest_use'] == 'on' ? '' : 'hideField'}}">
                    ...
                    <div class="input-row">
                        <input type="text" maxlength="100" name="date_of_retest" id="date_of_retest" value="{{$reprintData->date_of_retest}}"
                            class="restrict required validate input-large form-control form-control-sm enablefields"
                            autocomplete="off" placeholder="{{$config['date_format']}}"
                            {{$config['date_of_retest_mandatory'] == 'on' ? 'required' : ''}} readonly/>
                        <input type="{{$config['date_format'] == 'MMM-YYYY' ? 'month' : 'date'}}"
                            class=" form-control form-control-sm input-small past_date_validate enablefields" onChange="DateChange(this)" readonly/>
                    </div>
                </div>
                ...
                <label>{{$config['b_static_field1']}}
                    ...
                    <input type="varchar" maxlength="100" name="b_static_field1" id="b_static_field1"
                        class="required validate form-control form-control-sm enablefields" autocomplete="off"
            ...
            <div class="container-fluid" style="margin-top:1%;">
                <table id="container_table" class="table table-md table-bordered tablefix_mtop" style="width:100% !important;">
                    ...
                    <thead class="nonheadingfont-bold stickyhead" style="width:100% !important;top:-3.5% !important">
                        <tr>
                            <th><input type="checkbox" name="checkIdHead" value="" id="check_box_head" onclick="selectAll(this)"></th>
                            <th>S.No</th>
                    ...
                    <tbody>
                        ...
                        <tr id="{{$data->id}}">
                            ...
                            @endif
                            @if($config['d_field1_use'] == 'on')
                            <td><input type="text" readonly class="dynamic_field1 enablefields" value="{{$data->dynamic_field1}}" id="dynamic_field1_{{$data->id}}""></td>
                            @endif
                            @if($config['d_field2_use'] == 'on')
                            <td><input type="text" readonly class="dynamic_field2 enablefields" value="{{$data->dynamic_field2}}" id="dynamic_field2_{{$data->id}}"></td>
                            @endif
                        </tr>
                        @endforeach
                    </tbody>
...
function onInput(event) {
    ...
    let trId = event.closest('tr');
    let netWeight = parseFloat(trId.querySelector('#net_weight_' + trId.id).value) || 0;
    let tareWeight = parseFloat(trId.querySelector('#tare_weight_' + trId.id).value) || 0;
...
$('#edit_enablefields').click(function(){
        $('.enablefields').prop('readonly',false);
    });
    // Handle future dates
